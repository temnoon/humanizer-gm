...
// API URL for Gutenberg endpoints
const NPE_API_URL = import.meta.env.VITE_API_URL || 'https://npe-api.tem-527.workers.dev';
...
export function GutenbergView({ onSelectText, onClose }: GutenbergViewProps) {
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await fetch(
        `${NPE_API_URL}/gutenberg/search?q=${encodeURIComponent(searchQuery)}&languages=en`
      );
  ...
  const loadBookStructure = async (book: GutenbergBook) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${NPE_API_URL}/gutenberg/book/${book.id}/structure`);
      const data = await response.json();
      if (data.error) throw new Error(data.error);
  ...
  const loadSectionContent = async (sectionIndex: number) => {
    ...
    try {
      const response = await fetch(`${NPE_API_URL}/gutenberg/book/${selectedBook.id}/section/${sectionIndex}`);
      const data = await response.json();
      if (data.error) throw new Error(data.error);
