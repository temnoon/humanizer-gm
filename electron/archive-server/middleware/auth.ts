/**
 * JWT Authentication Middleware for Archive Server
 *
 * Validates JWT tokens from Authorization header and attaches
 * user context to request for downstream handlers.
 *
 * Copied from book-studio-server/middleware/auth.ts with minor adaptations.
...
export function initAuth(secret: string): void {
  ...
  console.log('[archive-server-auth] Auth middleware initialized');
}
...
async function validateToken(authHeader: string | undefined): Promise<AuthContext | null> {
  ...
  try {
    ...
    if (!userId) {
      console.warn('[archive-server-auth] Token missing user ID');
      return null;
    }
    ...
    if (error instanceof jose.errors.JWTExpired) {
      console.warn('[archive-server-auth] Token expired');
    } else if (error instanceof jose.errors.JWTClaimValidationFailed) {
      console.warn('[archive-server-auth] Token claim validation failed:', (error as Error).message);
    } else if (error instanceof jose.errors.JWSSignatureVerificationFailed) {
      console.warn('[archive-server-auth] Token signature verification failed');
    } else {
      console.warn('[archive-server-auth] Token validation error:', error);
    }
    return null;
  }
}
 ...
 *
 * Key behavior for multi-tenant support:
 * - If resource has no owner (NULL user_id = legacy data), allow access
 * - Admins can access everything
 * - Otherwise, user must match resource owner
 */
export function isOwner(req: Request, resourceUserId: string | null | undefined): boolean {
  const authReq = req as AuthenticatedRequest;
