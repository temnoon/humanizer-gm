...
import type { AgentMessage, HouseType, SignoffVote } from '../runtime/types';
...
export class ReviewerAgent extends AgentBase {
  ...
  // Reviewer's quality thresholds
  private thresholds = {
    minOverallScore: 0.7,
    minStyleScore: 0.65,
    minStructureScore: 0.6,
    minHumanizationScore: 0.7,
    minCitationScore: 0.8,
    blockingIssuesAllowed: 0,
    majorIssuesAllowed: 2,
  };
  ...
  private async reviewChapter(request: ReviewChapterRequest): Promise<ChapterReview> {
    ...
    if (blockingCount > this.thresholds.blockingIssuesAllowed) {
      verdict = 'reject';
      signoffVote = 'reject';
    } else if (majorCount > this.thresholds.majorIssuesAllowed || overallScore < this.thresholds.minOverallScore) {
      verdict = 'revise';
      signoffVote = 'pending';
    } else {
      verdict = 'approve';
...
Respond with JSON: {
  ...
  private async checkHumanization(request: CheckHumanizationRequest): Promise<HumanizationCheck> {
    ...
    return {
      ...
      aiDetectionRisk: humanScore >= 0.7 ? 'low' : humanScore >= 0.5 ? 'medium' : 'high',
...
Respond with JSON: { score: 0-1, issues: [{ severity, location, description }] }`,
  ...
  private async provideSignoff(request: ProvideSignoffRequest): Promise<SignoffDecision> {
    const { chapterId, projectId } = request;
  ...
  private async handleSignoffRequest(payload: SignoffRequestPayload): Promise<SignoffDecision> {
    const { signoffId, changeType, changeId, title, payload: changePayload } = payload;
