...
export class BuilderAgent extends AgentBase {
  ...
  // Builder's composition parameters
  private config = {
    minChapterWords: 1500,
    maxChapterWords: 5000,
    targetPacingScore: 0.7,
    voiceConsistencyThreshold: 0.6,
    maxBridgeLength: 200,          // Words for transition text
  };
  ...
  private async composeChapter(plan: CompositionPlan): Promise<ChapterDraft> {
    ...
    if (styleAnalysis.voiceConsistency < this.config.voiceConsistencyThreshold) {
      this.addIntention({
        type: 'revise',
        priority: 0.8,
    ...
    if (pacingScore < this.config.targetPacingScore) {
      this.addIntention({
        type: 'restructure',
        priority: 0.6,
...
Write in a consistent voice. Output only the composed text, no meta-commentary.`;
  ...
  private async generateTransition(
    fromSection: ChapterStructure['sections'][0],
    toContent: string,
    theme: string
  ): Promise<string> {
...
Output only the transition text.`;
    ...
    if (words.length > this.config.maxBridgeLength) {
      return words.slice(0, this.config.maxBridgeLength).join(' ') + '...';
    }
  ...
  private async analyzeStructure(request: AnalyzeStructureRequest): Promise<StructureAnalysis> {
...
Respond with JSON: {
  ...
  private async reviseDraft(request: ReviseDraftRequest): Promise<ChapterDraft> {
    const { chapterId, focusAreas, preservePassageRefs } = request;
...
Output the revised chapter text only.`,
  ...
  private async suggestImprovements(request: SuggestImprovementsRequest): Promise<ImprovementSuggestion[]> {
