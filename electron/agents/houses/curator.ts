...
import type { AgentMessage, HouseType, Proposal } from '../runtime/types';
...
export class CuratorAgent extends AgentBase {
  ...
  private concerns = {
    qualityThreshold: 0.6,      // Minimum quality to recommend
    gemThreshold: 0.85,         // Quality level for gem status
    redundancyThreshold: 0.8,   // Similarity level for redundancy
    coherenceThreshold: 0.5,    // Minimum thread coherence
  };
...
Respond with JSON: { clarity, depth, originality, relevance, concerns: [], recommendations: [] }`,
    ...
    const isGem = quality >= this.concerns.gemThreshold;
    ...
    if (quality < this.concerns.qualityThreshold) {
      this.addIntention({
        type: 'prune',
...
Respond with JSON: {
  ...
  suggestedOrder: [passage indices]
    ...
    const coherence: ThreadCoherence = {
      ...
      suggestedOrder: (Array.isArray(result.suggestedOrder) ? result.suggestedOrder : []).map((i: number) => passages[i]?.id).filter(Boolean),
    };

    // If coherence is low, propose reorganization
    if (coherence.coherenceScore < this.concerns.coherenceThreshold) {
      await this.proposeAction(
        'reorganize-thread',
  ...
  private async detectRedundancy(request: DetectRedundancyRequest): Promise<RedundancyReport> {
    const { passages, threshold = this.concerns.redundancyThreshold } = request;
    ...
    if (redundancies.length > 0) {
      await this.proposeAction(
        ...
        { redundancies, projectId: request.projectId },
        { projectId: request.projectId, requiresApproval: true }
      );
    }
...
Respond with JSON: { clusters: [{ theme, passageIndices: [], suggestedThreads: [] }] }`,
    ...
    const clusters: SemanticCluster[] = (Array.isArray(result.clusters) ? result.clusters : []).map((c, i: number) => ({
      id: `cluster-${i}`,
      theme: c.theme,
      passages: (Array.isArray(c.passageIndices) ? c.passageIndices : []).map((idx: number) => passages[idx]?.id).filter(Boolean),
      suggestedThreads: Array.isArray(c.suggestedThreads) ? c.suggestedThreads : [],
    }));

    // Propose new threads based on clusters
    for (const cluster of clusters) {
      if (cluster.passages.length >= 3 && cluster.suggestedThreads.length > 0) {
        await this.proposeAction(
  ...
  private async organizeThread(request: OrganizeThreadRequest): Promise<ThreadCoherence> {
    ...
    if (targetOrder) {
      ...
      const reorderedPassages = targetOrder.map(id => passages.find(p => p.id === id)!).filter(Boolean);
...
[
  ...
  private async callAI(capability: string, input: string, options?: { systemPrompt?: string }): Promise<string> {
    ...
    const modelMaster = this.bus.getAgent('model-master');
    if (!modelMaster) {
      throw new Error('Model Master agent not available');
    }
  ...
  private parseAnalysis(analysis: string): Record<string, unknown> {
    try {
