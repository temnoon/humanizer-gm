...
import { getAgentRegistry } from '../runtime/registry';
import { getMessageBus } from '../bus/message-bus';
import { getTaskQueue, type QueuedTask, type TaskQueueStats } from '../tasks/queue';
import {
  getAgentStore,
  type StoredSession,
  type StoredSignoff,
...
export interface CouncilOrchestrator {
  ...
  // Proposals
  submitProposal(agentId: string, action: ProposedAction): Promise<string>;
...
export class DefaultCouncilOrchestrator implements CouncilOrchestrator {
  ...
  async shutdown(): Promise<void> {
    ...
    const sessions = this.store.listSignoffs({ status: 'pending' });
    // Note: listSignoffs returns signoffs, we'd need a listSessions method
    // For now, just shutdown agents
  ...
  private determineRequiredAgents(changeType: string, config?: ProjectCouncilConfig): string[] {
  ...
  async submitProposal(agentId: string, action: ProposedAction): Promise<string> {
  ...
  getStats(): OrchestratorStats {
    ...
    return {
      ...
      registeredAgents: storeStats.agents,
      activeAgents,
    };
  }
