/**
 * JWT Authentication Middleware for NPE-Local Server
 *
...
export function initAuth(secret: string): void {
  ...
  console.log('[npe-local-auth] Auth middleware initialized');
}
...
async function validateToken(authHeader: string | undefined): Promise<AuthContext | null> {
  ...
  try {
    ...
    if (!userId) {
      console.warn('[npe-local-auth] Token missing user ID');
      return null;
    }
    ...
    if (error instanceof jose.errors.JWTExpired) {
      console.warn('[npe-local-auth] Token expired');
    } else if (error instanceof jose.errors.JWTClaimValidationFailed) {
      console.warn('[npe-local-auth] Token claim validation failed:', (error as Error).message);
    } else if (error instanceof jose.errors.JWSSignatureVerificationFailed) {
      console.warn('[npe-local-auth] Token signature verification failed');
    } else {
      console.warn('[npe-local-auth] Token validation error:', error);
    }
    return null;
  }
}
